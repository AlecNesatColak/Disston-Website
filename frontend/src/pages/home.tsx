...
export default function HomePage() {
  ...
  return (
    ...
    >
      ...
      {/* SVG Pattern Overlay */}
      <svg className="absolute inset-0 w-full h-full opacity-10 pointer-events-none" xmlns="http://www.w3.org/2000/svg" preserveAspectRatio="none" fill="none" viewBox="0 0 1440 900">
        <defs>
      ...
      <div className="absolute inset-0 pointer-events-none">
        <div className="absolute top-0 left-0 w-96 h-96 bg-blue-400 rounded-full mix-blend-multiply filter blur-2xl animate-pulse" style={{ opacity: 0.3 }} />
        <div className="absolute bottom-0 right-0 w-96 h-96 bg-yellow-300 rounded-full mix-blend-multiply filter blur-2xl animate-pulse" style={{ opacity: 0.3, animationDelay: "2s" }} />
        <div className="absolute top-1/2 left-1/2 w-80 h-80 bg-white rounded-full mix-blend-multiply filter blur-2xl animate-pulse" style={{ opacity: 0.15, animationDelay: "4s" }} />
      </div>
      ...
      <div className="absolute inset-0 opacity-5">
        ...
        <div className="absolute top-0 right-0 w-96 h-96 bg-purple-400 rounded-full mix-blend-multiply filter blur-xl animate-pulse" style={{ animationDelay: "2s" }} />
        <div className="absolute bottom-0 left-0 w-96 h-96 bg-green-400 rounded-full mix-blend-multiply filter blur-xl animate-pulse" style={{ animationDelay: "4s" }} />
      </div>
      ...
      <div className="absolute inset-0 pointer-events-none">
        <div className="absolute top-20 left-20 w-96 h-96 rounded-lg transform rotate-3 animate-float-slow overflow-hidden">
          <Image src="/images/Disston-team-pic.jpg" alt="Team Photo 2018" fill className="object-cover" />
        </div>
        <div className="absolute top-1/4 right-12 w-96 h-96 rounded-lg shadow-lg transform -rotate-2 animate-float-slow overflow-hidden" style={{ animationDelay: "3s" }}>
          <Image src="/images/Disston-team-pic2.jpg" alt="Team Photo 2019" fill className="object-cover" />
        </div>
        <div className="absolute top-1/3 left-4 w-96 h-96 rounded-lg shadow-lg transform rotate-1 animate-float-slow overflow-hidden" style={{ animationDelay: "1.5s" }}>
          <Image src="/images/gaffer.jpg" alt="Team Photo 2020" fill className="object-cover" />
        </div>
        <div className="absolute top-1/2 right-8 w-96 h-96 rounded-lg shadow-lg transform -rotate-1 animate-float-slow overflow-hidden" style={{ animationDelay: "4.5s" }}>
          <Image src="/images/Disston-Crests.jpg" alt="Team Photo 2021" fill className="object-cover" />
        </div>
        <div className="absolute bottom-1/4 left-8 w-96 h-96 rounded-lg shadow-lg transform -rotate-2 animate-float-slow overflow-hidden" style={{ animationDelay: "6s" }}>
          <Image src="/images/Banner.jpg" alt="Team Photo 2022" fill className="object-cover" />
        </div>
        <div className="absolute bottom-16 right-12 w-96 h-96 rounded-lg shadow-lg transform rotate-2 animate-float-slow overflow-hidden" style={{ animationDelay: "7.5s" }}>
          <Image src="/images/Banner.jpg" alt="Team Photo 2023" fill className="object-cover" />
        </div>
      </div>
      ...
      <div className="max-w-6xl mx-auto py-8 px-4 relative z-10">
        <div className="rounded-3xl bg-white/80 backdrop-blur-lg shadow-2xl border-2 border-blue-200/60 p-6 md:p-12">
          <div className="text-center mb-12">
            ...
            <p className="text-xl text-gray-600 mb-6">{HOMEPAGE_CONSTANTS.hero.subtitle}</p>
          ...
          {/* Latest Results (static example retained) */}
          <div className="bg-white/90 backdrop-blur-sm rounded-xl shadow-lg p-6 mb-8 border border-white/20">
            <h2 className="text-2xl font-bold text-gray-800 mb-6">{HOMEPAGE_CONSTANTS.sections.latestResults.title}</h2>
          ...
          {/* Upcoming Matches (static example retained) */}
          <div className="bg-white/90 backdrop-blur-sm rounded-xl shadow-lg p-6 mb-8 border border-white/20">
            <h2 className="text-2xl font-bold text-gray-800 mb-6">{HOMEPAGE_CONSTANTS.sections.upcomingMatches.title}</h2>
            <div className="space-y-4">
              <div className="flex items-center justify-between p-4 bg-gradient-to-r from-blue-50 to-blue-100 rounded-lg border border-blue-200 hover:shadow-md transition-shadow">
                <div className="flex items-center space-x-4">
                  <div className="text-sm text-gray-500">Sun, Dec 15</div>
          ...
          <div className="grid grid-cols-1 lg:grid-cols-2 gap-8 mb-8">
            <div className="bg-white/90 backdrop-blur-sm rounded-xl shadow-lg p-6 border border-white/20 hover:shadow-xl transition-all duration-300">
              <h3 className="text-xl font-bold text-gray-800 mb-4">{HOMEPAGE_CONSTANTS.sections.topScorers.title}</h3>
              <div className="space-y-3">
                {(isLoading ? [] : players.slice(0, 3)).map((player, index) => (
                  <div key={player.id ?? index} className="flex items-center justify-between p-3 bg-gradient-to-r from-yellow-50 to-yellow-100 rounded-lg border border-yellow-200">
            ...
            <div className="bg-white/90 backdrop-blur-sm rounded-xl shadow-lg p-6 border border-white/20 hover:shadow-xl transition-all duration-300">
              <h3 className="text-xl font-bold text-gray-800 mb-4">{HOMEPAGE_CONSTANTS.sections.cleanSheets.title}</h3>
              <div className="space-y-3">
                {(isLoading
                  ? []
                  : players
                      .filter((p) => ["GK", "LB", "RB", "CB"].includes(p.position ?? ""))
                      .slice()
                      .sort((a, b) => (b.clean_sheets ?? 0) - (a.clean_sheets ?? 0))
                      .slice(0, 3)
                ).map((player, index) => (
                  <div key={player.id ?? index} className="flex items-center justify-between p-3 bg-gradient-to-r from-blue-50 to-blue-100 rounded-lg border border-blue-200">
                    <div className="flex items-center space-x-3">
                      <div className="w-8 h-8 bg-blue-500 rounded-full flex items-center justify-center text-white font-bold">
                        {index + 1}
                      </div>
                    ...
                    <div className="font-bold text-blue-600">{player.clean_sheets}</div>
                  </div>
                ))}
              </div>
            </div>
          ...
          <div className="bg-gradient-to-r from-blue-600 via-purple-600 to-green-600 rounded-xl shadow-2xl p-8 text-center text-white relative overflow-hidden">
            ...
            <div className="relative z-10">
              <h2 className="text-3xl font-bold mb-4">{HOMEPAGE_CONSTANTS.sections.joinClub.title}</h2>
              <p className="text-xl mb-6">{HOMEPAGE_CONSTANTS.sections.joinClub.subtitle}</p>
              <div className="flex justify-center space-x-4">
                <Link href="/player-request" className="bg-white text-blue-600 px-6 py-3 rounded-lg font-semibold hover:bg-gray-100 transition-all duration-300 hover:scale-105 shadow-lg inline-block">
                  Join Team
                </Link>
